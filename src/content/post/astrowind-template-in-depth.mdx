...
title: Purple Martin template in depth
excerpt: While easy to get started, Purple Martin is quite complex internally.  This page provides documentation on some of the more intricate parts.
...
It can be a somewhat daunting task trying to get a handle on _Purple Martin_ internals, and particularly various points of usage.

This page outlines and clarifies some of the techniques found in _Purple Martin_. Use it as a guide for further modification, or an instructional for techniques to use in your own endeavors.

## Styling

As the name suggests, _Purple Martin_ relies on _TailWind_ for styling. Furthermore, _Purple Martin_ defines custom low level style settings which are incorporated into _TailWind_ seamlessly, and which provides consistency for higher level styling constructs, as well as enabling dark mode.
...
<DListItem dt="layouts/Layout.astro">
  This layout is used for all of the pages rendered by _Purple Martin_. The contents of _tailwind.css_ and
...
Now, we get to the techniques used in _Purple Martin_, we'll use the _pages/index.astro_ file to illustrate.
